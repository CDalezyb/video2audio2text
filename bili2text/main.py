...
def main():
    ...
    for idx, av in enumerate(av_list, 1):
        ...
        try:
            slice_directories = batch_extract_and_split_audio(downloaded_files)
            if not slice_directories:
                print(f"警告：BV号{av}音频处理失败，跳过转文字")
                continue
        except Exception as e:
        ...
        try:
            txt_files = run_analysis(
                slice_dirs=slice_directories,
                prompt="以下是普通话的句子。"  # 提升中文转写准确率的提示
            )
            all_txt_files.extend(txt_files)
            print(f"✅ BV号{av}处理完成，生成文本文件：{txt_files}")
        except Exception as e:
            print(f"错误：BV号{av}转文字失败，原因：{str(e)}")
            continue
